<script lang="ts">
	...
	let nama = $state(data.divisi.nama);
	let slug = $state(data.divisi.slug);
	let deskripsi = $state(data.divisi.deskripsi || '');
	let ikon = $state(data.divisi.ikon || '');
	let gambarBanner = $state(data.divisi.gambarBanner || '');
	let urutan = $state(String(data.divisi.urutan || 0));
	let status = $state<'aktif' | 'nonaktif'>(data.divisi.status);
	let loading = $state(false);

	$effect(() => {
		if (form?.success !== undefined) {
			if (form.success) {
				toast.success(form.message || 'Divisi berhasil diperbarui');
...
<svelte:head>
	<title>Edit Divisi - Dashboard HIMA</title>
</svelte:head>

<div class="space-y-6">
	<div class="flex items-center justify-between">
		<div>
			<Breadcrumb items={[{ label: 'Divisi', href: '/dashboard/divisi' }, { label: 'Edit' }]} />
			<h1 class="mt-2 text-2xl font-bold text-slate-900">Edit Divisi</h1>
		</div>
