<script lang="ts">
	...
	interface Props {
		data: {
			struktur: { id: number; nama: string; jabatan: string; divisiId: number | null; foto: string | null; email: string | null; instagram: string | null; linkedin: string | null; periode: string | null; urutan: number; status: 'aktif' | 'nonaktif' };
	...
	let nama = $state(data.struktur.nama);
	let jabatan = $state(data.struktur.jabatan);
	let divisiId = $state(data.struktur.divisiId ? String(data.struktur.divisiId) : '');
	let foto = $state(data.struktur.foto || '');
	let email = $state(data.struktur.email || '');
	let instagram = $state(data.struktur.instagram || '');
	let linkedin = $state(data.struktur.linkedin || '');
	let periode = $state(data.struktur.periode || '');
	let urutan = $state(String(data.struktur.urutan));
	let status = $state<'aktif' | 'nonaktif'>(data.struktur.status);
...
<svelte:head><title>Edit Anggota - Dashboard HIMA</title></svelte:head>

<div class="space-y-6">
	<div class="flex items-center justify-between">
		<div>
			<Breadcrumb items={[{ label: 'Struktur Organisasi', href: '/dashboard/struktur-organisasi' }, { label: 'Edit' }]} />
			<h1 class="mt-2 text-2xl font-bold text-slate-900">Edit Anggota</h1>
		</div>
	...
	<form method="POST" use:enhance={() => { loading = true; return async ({ update }) => await update(); }} class="grid gap-6 lg:grid-cols-3">
		<input type="hidden" name="id" value={data.struktur.id} />
			...
			<div class="rounded-xl border border-slate-200 bg-white p-6 space-y-4">
				<div class="grid gap-4 sm:grid-cols-2">
					...
					<div>
						...
						<input type="text" id="jabatan" name="jabatan" bind:value={jabatan} required class="w-full rounded-lg border border-slate-300 px-4 py-2.5 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20" />
					</div>
				</div>
				<div class="grid gap-4 sm:grid-cols-2">
					<div>
					...
					<div>
						...
						<input type="text" id="periode" name="periode" bind:value={periode} class="w-full rounded-lg border border-slate-300 px-4 py-2.5 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20" />
					</div>
				</div>
				<div>
				...
				<div class="grid gap-4 sm:grid-cols-2">
					<div>
						...
						<input type="text" id="instagram" name="instagram" bind:value={instagram} class="w-full rounded-lg border border-slate-300 px-4 py-2.5 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20" />
					</div>
					<div>
						<label for="linkedin" class="mb-1.5 block text-sm font-medium text-slate-700">LinkedIn</label>
						<input type="text" id="linkedin" name="linkedin" bind:value={linkedin} class="w-full rounded-lg border border-slate-300 px-4 py-2.5 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20" />
					</div>
				</div>
			</div>
		</div>
		...
		<div class="space-y-6">
			<div class="rounded-xl border border-slate-200 bg-white p-6 space-y-4">
				...
				<Button type="submit" fullWidth {loading}>{#snippet icon()}<Save class="h-4 w-4" />{/snippet}Simpan Perubahan</Button>
			</div>
